...
- **`DuckDBLoader`** (duckdb / motherduck) — embedded analytical SQL database;
  in-memory, persistent file, or MotherDuck cloud; append / replace / upsert
  (ON CONFLICT); query() helper.
- **`ParquetLoader`** (parquet) — write Parquet files to local disk, S3, GCS,
  or Azure Blob via pyarrow; supports partitioned datasets and snappy/gzip/zstd
  compression.
- **`DeltaLakeLoader`** (deltalake) — open table format for Databricks/Spark;
  append / replace / MERGE upsert via python-deltalake; supports cloud storage
  with storage_options.
- **`IcebergLoader`** (iceberg) — Apache Iceberg open table format; REST, Glue,
  Hive, and SQL catalog backends via PyIceberg; append / replace.
- **`S3Loader`** (s3 / gcs / azure_blob) — write CSV, JSON, JSONL, or Parquet
  to Amazon S3, Google Cloud Storage, or Azure Blob Storage.
- **`AthenaLoader`** (athena) — write to AWS Athena via S3 Parquet staging and
  MSCK REPAIR TABLE; waits for query completion.
- **`SFTPLoader`** (sftp) — upload CSV, JSON, JSONL, or Parquet to remote SFTP
  servers via Paramiko; supports password or private key authentication.
- **`MicrosoftFabricLoader`** (fabric) — write Parquet or Delta Lake files to
  Microsoft Fabric Lakehouses via OneLake (ADLS Gen2 API).
- **`PostGISLoader`** (postgis) — PostgreSQL with PostGIS geospatial extension;
  writes WKT geometry strings as native PostGIS geometry columns via
  ST_GeomFromText(); CREATE EXTENSION IF NOT EXISTS postgis automatic.
