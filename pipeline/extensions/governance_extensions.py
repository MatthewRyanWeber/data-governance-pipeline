...
class RetentionEnforcer:
    ...
    def enforce(
        ...
        result: Dict = {"rows_found": 0, "rows_acted": 0, "action": action, "dry_run": dry_run}
        ...
        rows_acted = int(result["rows_acted"])
        if rows_acted > 0:
            self.gov.erasure_executed(
                f"retention_{policy}", table, rows_acted, "RETENTION_DELETE"
            )
        self.gov._event(  # type: ignore[attr-defined]
...
class PseudonymVault:
    ...
    def _encrypt(self, value: str) -> str:
        return str(self._fernet.encrypt(value.encode()).decode())
...
def demo(output_dir: Optional[str] = None) -> None:
    ...
    gov.retention_policy("30_days", 30)
