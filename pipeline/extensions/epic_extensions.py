...
class HIPAASafeHarborFilter:
    ...
    def _build_html(self) -> str:
        ...
        if r is None:
            raise RuntimeError(
                "PHI scan report not available — run scan() before "
...
class ClarityExtractor:
    ...
    def list_tables(self) -> List[str]:
        ...
        inspector = sa_inspect(self.engine())
        return list(inspector.get_table_names(schema=self.schema))
...
class IRBApprovalGate:
    ...
    def _load_registry(self) -> Dict:
        if self._reg_path.exists():
            try:
                return dict(json.loads(self._reg_path.read_text(encoding="utf-8")))
            except Exception as exc:  # pylint: disable=broad-exception-caught
...
class PHIKAnonymityChecker:
    ...
    def _build_html(self) -> str:
        ...
        if r is None:
            raise RuntimeError(
                "k-anonymity report not available — run check() before "
                "writing the HTML report."
            )
        buf  = io.StringIO()
