...
# MotherDuck — cloud-hosted DuckDB (uses same duckdb package)
# HAS_DUCKDB covers both DuckDB and MotherDuck

# Delta Lake — open table format (Databricks, Spark ecosystem)
try:
    import deltalake as _deltalake                   # noqa: F401
    HAS_DELTALAKE = True
except ImportError:
    HAS_DELTALAKE = False

# Apache Iceberg — open table format (AWS, Snowflake, Spark)
try:
    import pyiceberg as _pyiceberg                   # noqa: F401
    HAS_ICEBERG = True
except ImportError:
    HAS_ICEBERG = False

# Amazon S3 / GCS / Azure Blob — object storage destinations
try:
    import boto3 as _boto3_dest                      # noqa: F401
    HAS_S3 = True
except ImportError:
    HAS_S3 = False

# SFTP — write files to remote SFTP servers
try:
...
# AWS Athena — serverless SQL over S3 data lakes
# Uses boto3 (same as S3) — HAS_S3 covers Athena too

# Microsoft Fabric / OneLake — Azure Data Lake Storage Gen2
try:
    import adlfs as _adlfs                           # noqa: F401
    HAS_FABRIC = True
except ImportError:
    HAS_FABRIC = False

# PostGIS — PostgreSQL + geospatial extension
# Uses SQLAlchemy + psycopg2 (already available) — no extra package needed
HAS_POSTGIS = True   # always available via existing psycopg2/sqlalchemy
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: DuckDBLoader  (v1.0)
#  Write governed DataFrames to DuckDB — the embedded analytical SQL database.
# ═════════════════════════════════════════════════════════════════════════════

class DuckDBLoader:
    """
    Write a governed, PII-masked DataFrame to DuckDB.

    DuckDB is an embedded in-process analytical database — no server, no
    Docker, no configuration.  A single ``pip install duckdb`` gives you a
    fully featured column-store SQL engine that runs inside your Python process.
    It is the most popular local analytical database in the Python data
    engineering ecosystem and the default destination in dlt.

    Three connection modes
    ─────────────────────
    In-memory   ``db_path=":memory:"``
        Data lives only for the duration of the connection.  Useful for testing
        and intermediate transformations.

    Persistent  ``db_path="/path/to/data.duckdb"``
        Data is saved to a file and survives process restarts.

    MotherDuck  ``db_path="md:my_database"``
        Cloud-hosted DuckDB via MotherDuck.  Requires a MotherDuck token in
        the environment (``MOTHERDUCK_TOKEN``) or via cfg["motherduck_token"].

    Required cfg keys
    -----------------
    db_path     : str   DuckDB file path, ":memory:", or "md:database_name"
    ...
    ----------
    append   (default)
        INSERT INTO — adds rows to existing table or creates it.

    replace
        DROP TABLE IF EXISTS then CREATE TABLE AS SELECT.
    ...
    Quick-start
    ───────────
        loader = DuckDBLoader(gov)
        rows   = loader.load(df, cfg={"db_path": "./analytics.duckdb"},
                             table="employees")
        # MotherDuck
        rows   = loader.load(df, cfg={"db_path": "md:my_db",
                                      "motherduck_token": "..."}, table="emps")
    """
    ...
    def load(
        ...
        table         Target table name.
        if_exists     "append" | "replace" | "upsert"
        natural_keys  Key columns for upsert ON CONFLICT clause.
        ...
        try:
            if if_exists == "replace":
                ...
                for k in natural_keys:
                    _validate_sql_identifier(k, "natural_key")
                missing = [k for k in natural_keys if k not in df.columns]
                if missing:
                    raise ValueError(
                        f"DuckDBLoader: upsert key(s) not in DataFrame: {missing}"
                    )
                # Create table if not exists, then INSERT OR REPLACE
                conn.execute(
    ...
    def query(self, cfg: dict, sql: str) -> "pd.DataFrame":
        ...
        Parameters
        ----------
        cfg   Connection config with db_path.
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: ParquetLoader  (v1.0)
#  Write governed DataFrames to Parquet files on local disk or cloud storage.
# ═════════════════════════════════════════════════════════════════════════════

class ParquetLoader:
    """
    Write a governed, PII-masked DataFrame to Parquet files.

    Parquet is the standard columnar file format for data lakes and
    analytical workloads.  Writing to Parquet is often the first step
    before loading into Delta Lake, Iceberg, Athena, or BigQuery.

    Destinations
    ────────────
    Local disk      ``path="/local/path/output.parquet"``
    Amazon S3       ``path="s3://bucket/prefix/output.parquet"``
    Google Cloud    ``path="gs://bucket/prefix/output.parquet"``
    Azure Blob      ``path="az://container/prefix/output.parquet"``

    Cloud paths require the appropriate fsspec-compatible library:
    S3 → ``pip install s3fs``
    ...
    Required cfg keys
    -----------------
    path        : str   Output file path (local or cloud URI).

    Optional cfg keys
    -----------------
    compression : str   "snappy" (default) | "gzip" | "zstd" | "none"
    row_group_size : int  Rows per Parquet row group (default 128,000).
    partition_cols : list  Columns to partition output by (creates directory tree).
    storage_options : dict  Cloud auth options passed to fsspec/pyarrow.
                           e.g. {"key": "...", "secret": "..."} for S3.

    Load modes
    ----------
    append   (default)
        Writes a new file.  If path already exists, raises unless replace mode.
        For partitioned datasets, appends new partition files.

    replace
        Overwrites the existing file or partition directory.
    ...
    Quick-start
    ───────────
        loader = ParquetLoader(gov)
        rows   = loader.load(df, cfg={"path": "./output.parquet"})
        # Partitioned S3
        rows   = loader.load(df, cfg={
    ...
    def load(
        ...
        df:           "pd.DataFrame",
        cfg:          dict,
        table:        str       = "",   # used as filename stem if path not in cfg
        if_exists:    str       = "append",
        natural_keys: "list | None" = None,  # noqa: F841 — not applicable
    ) -> int:
        """
        ...
        ----------
        df          DataFrame to write.
        cfg         Config dict (see class docstring).
        table       Used as filename stem if cfg["path"] not provided.
        if_exists   "append" | "replace"
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: DeltaLakeLoader  (v1.0)
#  Write governed DataFrames to Delta Lake — the open table format for
#  Databricks, Spark, and cloud data lake workloads.
# ═════════════════════════════════════════════════════════════════════════════

class DeltaLakeLoader:
    """
    Write a governed, PII-masked DataFrame to a Delta Lake table.

    Delta Lake is an open table format that adds ACID transactions, schema
    enforcement, time travel (versioned snapshots), and upsert (MERGE)
    support to Parquet files stored on object storage or local disk.  It is
    the native table format for Databricks and is widely used with Apache
    Spark, Apache Flink, and dbt.

    Delta tables are just directories of Parquet files plus a ``_delta_log``
    transaction log — no server or external service required.

    Destinations
    ────────────
    Local disk      ``path="/local/path/delta_table/"``
    Amazon S3       ``path="s3://bucket/prefix/"``
    Azure ADLS Gen2 ``path="az://container/prefix/"``
    Google Cloud    ``path="gs://bucket/prefix/"``

    Required cfg keys
    -----------------
    path            : str   Path to the Delta table root directory.

    Optional cfg keys
    -----------------
    storage_options : dict  Cloud auth credentials for pyarrow / fsspec.
                           S3: {"AWS_ACCESS_KEY_ID": "...", "AWS_SECRET_ACCESS_KEY": "..."}
                           Azure: {"AZURE_STORAGE_ACCOUNT_NAME": "...", "AZURE_STORAGE_ACCOUNT_KEY": "..."}
    schema_mode     : str   "merge" | "overwrite" — how to handle schema changes.

    Load modes
    ----------
    append   (default)
        Add rows to the Delta table.  Existing data is not modified.

    replace
        Overwrite the entire table.  All existing data is replaced.

    upsert   (natural_keys provided)
        MERGE INTO — update matching rows and insert new ones.
        Requires natural_keys to identify which rows to update.
    ...
    Quick-start
    ───────────
        loader = DeltaLakeLoader(gov)
        rows   = loader.load(df, cfg={"path": "./delta_employees/"},
                             table="employees")
        # With upsert
        rows   = loader.load(df, cfg={"path": "s3://my-bucket/employees/",
                                      "storage_options": {...}},
                             if_exists="upsert", natural_keys=["employee_id"])
    """
    ...
    def load(
        ...
        df:           "pd.DataFrame",
        cfg:          dict,
        table:        str       = "",   # unused — Delta uses path
        if_exists:    str       = "append",
        natural_keys: "list | None" = None,
    ) -> int:
        """
        ...
        df            DataFrame to write.
        cfg           Config dict (see class docstring).
        table         Unused — Delta tables are addressed by path.
        if_exists     "append" | "replace" | "upsert"
        natural_keys  Key columns for MERGE upsert predicate.
        ...
        if if_exists == "upsert" and natural_keys:
            ...
            if missing:
                raise ValueError(
                    f"DeltaLakeLoader: upsert key(s) not in DataFrame: {missing}"
                )
            predicate = " AND ".join(
                f"t.{k} = s.{k}" for k in natural_keys
            )
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: IcebergLoader  (v1.0)
#  Write governed DataFrames to Apache Iceberg tables via PyIceberg.
# ═════════════════════════════════════════════════════════════════════════════

class IcebergLoader:
    """
    Write a governed, PII-masked DataFrame to an Apache Iceberg table.

    Apache Iceberg is an open table format designed for huge analytic
    datasets.  It powers the lakehouse architecture in AWS (Glue, Athena),
    Snowflake (Iceberg tables), Spark, and Flink.  Iceberg tables are stored
    as Parquet files with a separate metadata layer that tracks schema
    evolution, partition evolution, and time travel.

    PyIceberg is the official Python client.  It supports multiple catalog
    backends: REST, AWS Glue, Hive, SQL (SQLite/PostgreSQL), and in-memory.

    Required cfg keys
    -----------------
    catalog_type   : str   "rest" | "glue" | "hive" | "sql" | "memory"
    catalog_name   : str   Catalog name (e.g. "default")
    namespace      : str   Iceberg namespace / database (e.g. "prod")
    table_name     : str   Iceberg table name.
    warehouse      : str   Warehouse URI (S3 path, local path, etc.)

    Optional cfg keys  (vary by catalog_type)
    -----------------
    uri            : str   Catalog REST endpoint URL.
    token          : str   REST catalog bearer token.
    region         : str   AWS region (for Glue catalog).
    access_key_id  : str   AWS credentials (for Glue/S3).
    secret_access_key : str
    thrift_uri     : str   Hive metastore thrift URI.
    catalog_db     : str   SQL catalog database connection string.

    Load modes
    ----------
    append   (default)   Append rows to the Iceberg table.
    ...
    def load(
        ...
        ----------
        df          DataFrame to write.
        cfg         Catalog and table configuration (see class docstring).
        table       Optional table name override (overrides cfg["table_name"]).
        if_exists   "append" | "replace"
    ...
    def _load_catalog(cfg: dict):
        ...
        if cfg.get("catalog_db"):
            # For SQL catalog: warehouse path also needed
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: S3Loader  (v1.0)
#  Write governed DataFrames to Amazon S3 / GCS / Azure Blob as files.
# ═════════════════════════════════════════════════════════════════════════════

class S3Loader:
    """
    Write a governed, PII-masked DataFrame to Amazon S3, Google Cloud
    Storage, or Azure Blob Storage as CSV, JSON, or Parquet files.

    This loader handles the common data lake pattern of writing structured
    data to object storage as flat files, which can then be queried via
    Athena, BigQuery external tables, or Snowflake stages.

    Required cfg keys
    -----------------
    bucket      : str   Bucket or container name.
    key         : str   Object key / path within the bucket.
                        e.g. "data/employees/2024-01-01.parquet"
    provider    : str   "s3" | "gcs" | "azure"

    Optional cfg keys
    -----------------
    format          : str   "parquet" (default) | "csv" | "json" | "jsonl"
    compression     : str   "snappy" (parquet) | "gzip" | "none"
    region          : str   AWS region (default "us-east-1")
    aws_access_key  : str   AWS credentials (falls back to env/IAM role)
    aws_secret_key  : str
    gcs_credentials : str   Path to GCS service account JSON key
    azure_account   : str   Azure storage account name
    azure_key       : str   Azure storage account key
    storage_options : dict  Passed directly to fsspec / s3fs / gcsfs / adlfs

    Requirements
    ------------
        pip install boto3 pyarrow        # for S3
        pip install gcsfs pyarrow        # for GCS
        pip install adlfs pyarrow        # for Azure
    ...
    ───────────
        ...
        rows   = loader.load(df, cfg={
            "provider": "s3",
            "bucket":   "my-data-lake",
            "key":      "employees/2024-01-15.parquet",
        })
    """
    ...
    def load(
        ...
        df:           "pd.DataFrame",
        cfg:          dict,
        table:        str       = "",   # unused — S3 uses bucket/key
        if_exists:    str       = "replace",
        natural_keys: "list | None" = None,  # noqa: F841
    ) -> int:
        """
        ...
        ----------
        df          DataFrame to write.
        cfg         Storage config (see class docstring).
        table       Unused — S3 objects are addressed by bucket + key.
        if_exists   "replace" (default) | "append" (appends rows to CSV/JSON)
                    Note: Parquet files are always overwritten by key.
        ...
        if not key:
            raise ValueError(
                "S3Loader: supply object key via cfg['key'] or the table parameter."
            )
        if fmt not in self._FORMATS:
            raise ValueError(
                f"S3Loader: format must be one of {self._FORMATS}, got '{fmt}'."
            )
    ...
    @staticmethod
    def _serialise(df: "pd.DataFrame", fmt: str, cfg: dict) -> bytes:
        import io
        if fmt == "parquet":
    ...
    @staticmethod
    def _upload(body: bytes, provider: str, bucket: str, key: str, cfg: dict) -> None:
        if provider == "s3":
            import boto3 as _b3
            kwargs: dict = {}
            if cfg.get("region"):
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: AthenaLoader  (v1.0)
#  Write governed DataFrames to AWS Athena via S3 staging + CTAS / MSCK.
# ═════════════════════════════════════════════════════════════════════════════

class AthenaLoader:
    """
    Write a governed, PII-masked DataFrame to AWS Athena.

    Athena is a serverless SQL engine that queries data stored in S3 using
    the Hive/Presto SQL dialect.  There is no dedicated Athena write API —
    data is loaded by writing Parquet files to S3 and then registering or
    refreshing the Athena table metadata.

    Architecture
    ────────────
    1. Write df as Parquet to the S3 staging location.
    2. Run ``CREATE TABLE AS SELECT`` (CTAS) or ``MSCK REPAIR TABLE``
       to make the data queryable in Athena.
    3. Query results via the Athena Query Execution API.
    ...
    s3_data_dir     : str   S3 path where the Parquet data will be stored.
                            e.g. "s3://my-bucket/data/employees/"
    region          : str   AWS region, e.g. "us-east-1"

    Optional cfg keys
    -----------------
    aws_access_key  : str   Falls back to environment / IAM role.
    aws_secret_key  : str
    ...
    Quick-start
    ───────────
        loader = AthenaLoader(gov)
        rows   = loader.load(df, cfg={
            "database":       "prod",
            "s3_staging_dir": "s3://my-bucket/athena-results/",
            "s3_data_dir":    "s3://my-bucket/data/employees/",
            "region":         "us-east-1",
        }, table="employees")
    """
    ...
    def load(
        ...
        if not s3_staging:
            raise ValueError("AthenaLoader: cfg must contain 's3_staging_dir'.")
        ...
        s3_client.put_object(Bucket=bucket_name, Key=file_key, Body=buf.getvalue())

        # Repair / refresh Athena table metadata
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: SFTPLoader  (v1.0)
#  Write governed DataFrames to remote servers via SFTP.
# ═════════════════════════════════════════════════════════════════════════════

class SFTPLoader:
    """
    Write a governed, PII-masked DataFrame to a remote SFTP server.

    SFTP is still widely used for data exchange with legacy systems,
    healthcare partners (HL7/EDI file drops), financial data vendors,
    and any on-premise system that does not expose a REST API.

    The DataFrame is serialised to CSV, JSON, JSONL, or Parquet and
    uploaded to the specified remote path using Paramiko.

    Required cfg keys
    -----------------
    host        : str   SFTP server hostname or IP.
    username    : str   SSH username.
    remote_path : str   Full path on the remote server.
                        e.g. "/data/uploads/employees.csv"

    One of:
    password    : str   SSH password.
    private_key : str   Path to private key file (e.g. "~/.ssh/id_rsa").
    private_key_passphrase : str  Passphrase for encrypted private key.

    Optional cfg keys
    -----------------
    port        : int   SSH port (default 22).
    format      : str   "csv" (default) | "json" | "jsonl" | "parquet"
    compression : str   Parquet compression (default "snappy").
    timeout     : int   Connection timeout in seconds (default 30).

    Requirements
    ------------
        pip install paramiko pyarrow

    Quick-start
    ───────────
        loader = SFTPLoader(gov)
        rows   = loader.load(df, cfg={
            "host":        "sftp.partner.com",
            "username":    "datauser",
            "password":    "secret",
            "remote_path": "/uploads/employees.csv",
        })
    """
    ...
    def load(
        ...
        df:           "pd.DataFrame",
        cfg:          dict,
        table:        str       = "",   # used as filename if remote_path not set
        if_exists:    str       = "replace",
        natural_keys: "list | None" = None,  # noqa: F841
    ) -> int:
        """
        ...
        cfg         Connection config (see class docstring).
        table       Used as the remote filename if cfg["remote_path"] not set.
        if_exists   "replace" (default) — remote files are always overwritten.
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: MicrosoftFabricLoader  (v1.0)
#  Write governed DataFrames to Microsoft Fabric via OneLake (ADLS Gen2).
# ═════════════════════════════════════════════════════════════════════════════

class MicrosoftFabricLoader:
    """
    Write a governed, PII-masked DataFrame to Microsoft Fabric via OneLake.

    Microsoft Fabric is Microsoft's unified analytics platform combining
    OneLake (data lake), Lakehouse (Delta Lake on OneLake), Synapse, and
    Power BI in one service.  OneLake exposes an Azure Data Lake Storage
    Gen2 (ADLS Gen2) compatible API, which means you can write to Fabric
    Lakehouses using the ``adlfs`` library with OneLake-specific credentials.

    Architecture
    ────────────
    Data is written as Parquet or Delta Lake files to a OneLake Lakehouse.
    The Lakehouse makes the data immediately queryable via Fabric SQL
    analytics endpoint, Power BI Direct Lake mode, and Spark notebooks.

    Required cfg keys
    -----------------
    workspace_id  : str   Fabric workspace GUID.
    lakehouse_id  : str   Lakehouse GUID within the workspace.
    path          : str   Path inside the Lakehouse Files section.
                          e.g. "Files/data/employees/"
    token         : str   Azure AD bearer token with Fabric scope
                          (or use DefaultAzureCredential via environment).

    Optional cfg keys
    -----------------
    format       : str   "parquet" (default) | "delta"
                         "delta" writes a Delta Lake table to the Tables section.
    account_name : str   OneLake account name (default "onelake").
    if_exists    : str   "append" | "replace"

    Requirements
    ------------
        pip install adlfs pyarrow deltalake

    Quick-start
    ───────────
        loader = MicrosoftFabricLoader(gov)
        rows   = loader.load(df, cfg={
            "workspace_id": "00000000-...",
            "lakehouse_id": "00000000-...",
            "path":         "Files/data/employees/",
    ...
    def load(
        ...
        df          DataFrame to write.
        cfg         Config dict (see class docstring).
        table       Used as filename/table name within the path.
        if_exists   "replace" (default) | "append"
        ...
        if not workspace_id:
            raise ValueError("MicrosoftFabricLoader: cfg must contain 'workspace_id'.")
        if not lakehouse_id:
            raise ValueError("MicrosoftFabricLoader: cfg must contain 'lakehouse_id'.")

        if df.empty:
            return 0

        # OneLake path structure:
        # {workspace_id}/{lakehouse_id}.Lakehouse/{path}/{table}.parquet
...
# ═════════════════════════════════════════════════════════════════════════════
#  CLASS: PostGISLoader  (v1.0)
#  Write governed DataFrames with geometry columns to PostgreSQL + PostGIS.
# ═════════════════════════════════════════════════════════════════════════════

class PostGISLoader:
    """
    Write a governed, PII-masked DataFrame with geometry columns to a
    PostgreSQL database with the PostGIS extension enabled.

    PostGIS adds native geographic and geometric data types (Point, Polygon,
    LineString, etc.) to PostgreSQL, enabling spatial queries like
    ``ST_Within``, ``ST_Distance``, ``ST_Intersects``, and full GIS
    analysis without leaving SQL.

    Architecture
    ────────────
    DataFrame geometry columns must contain WKT (Well-Known Text) strings
    such as ``"POINT(-74.006 40.7128)"`` or WKB hex strings.  The loader
    converts them to PostGIS geometry via ``ST_GeomFromText()`` on insert.

    All non-geometry columns are written normally via pandas to_sql; the
    geometry column is then updated with the correct PostGIS type via
    an ``ALTER TABLE … ALTER COLUMN … TYPE geometry`` statement.
    ...
    -----------------
    port         : int   Default 5432.
    geometry_col : str   Column containing WKT geometry strings.
                         Default "geometry".
    ...
    ───────────
        ...
        df = pd.DataFrame({
            ...
            "geometry": ["POINT(-73.9855 40.7580)", "POINT(-73.9654 40.7851)"],
        })
        loader = PostGISLoader(gov)
        rows   = loader.load(df, cfg={
    ...
    def __init__(self, gov: "GovernanceLogger") -> None:
        self.gov = gov
        # Uses psycopg2 + sqlalchemy — always available if project is installed
    ...
    def load(
        ...
        Parameters
        ----------
        df          DataFrame to write (must contain a WKT geometry column).
        cfg         Connection config (see class docstring).
        table       Target table name.
        if_exists   "append" | "replace"
        ...
        # Write non-geometry columns first via pandas
        df_meta = df.drop(columns=[geom_col]) if geom_col in df.columns else df
        df_meta.to_sql(table, engine, if_exists=if_exists,
                       index=False, method="multi", chunksize=500)

        # Add geometry column and populate via ST_GeomFromText
...
_LOADER_DISPATCH.update({
    ...
    "duckdb":       (DuckDBLoader,        False, False),
    "motherduck":   (DuckDBLoader,        False, False),
    "parquet":      (ParquetLoader,       False, False),
    "deltalake":    (DeltaLakeLoader,     False, False),
    "iceberg":      (IcebergLoader,       False, False),
    "s3":           (S3Loader,            False, False),
    "gcs":          (S3Loader,            False, False),
    "azure_blob":   (S3Loader,            False, False),
    "athena":       (AthenaLoader,        False, False),
    "sftp":         (SFTPLoader,          False, False),
    "fabric":       (MicrosoftFabricLoader, False, False),
    "postgis":      (PostGISLoader,       False, False),
