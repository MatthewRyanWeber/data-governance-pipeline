...
try:
    from catalog_connectors import (
        CatalogManager,
...
try:
    from metadata_extensions import (
        MetadataExtensionOrchestrator,
        prompt_metadata_extensions_config,
        SchemaDriftError,
        DataFreshnessError,
...
class GovernanceLogger:
    ...
    def _event(
        ...
        entry = base_entry  # for the logging call below
...
class Transformer:
    ...
    def standardise_names(self, df: "pd.DataFrame") -> "pd.DataFrame":
        ...
        import re as _re
        df = df.copy()
        df.columns = [_re.sub(r"[^a-z0-9_]", "", c.lower().replace(" ","_")) for c in df.columns]
        return df
...
class HanaLoader:
    ...
    def load(
        ...
        """
        import pandas as pd  # local to avoid top-level dep at import time
...
def run_governance_preflight(  # noqa: C901
    ...
    if "col_purpose" in disc_map:
        ...
        if matching:
            print(f"  Registered purposes for columns in this dataset:")
    ...
    if "purpose_limit" in disc_map:
        ...
        if purposes and _yn("  Enforce purpose limitations on this load?", default=True):
            ...
            print(f"  Registered purposes:")
            for idx, p in enumerate(purposes, 1):
                print(f"    {idx}. {p}")
            try:
                raw = input(f"  Which purpose applies to this load? [1]: ").strip()
