...
class TestHIPAASafeHarborFilter(_TmpMixin):
    ...
    def test_save_report_html(self):
        ...
        self.assertIn("HIPAA", out.read_text(encoding="utf-8"))
...
class TestBAATracker(_TmpMixin):
    ...
    def test_export_register_html(self):
        ...
        self.assertTrue(out.exists())
        self.assertIn("VendorX", out.read_text(encoding="utf-8"))
    ...
    def test_register_baa_invalid_expiry_date_raises(self):
        """Malformed expiry_date must raise ValueError on register, not later."""
        t = self._tracker()
        with self.assertRaises(ValueError) as ctx:
            t.register_baa(
                destination_id="bad_dest",
                vendor="V",
                signed_date="2024-01-01",
    ...
    def test_register_baa_invalid_signed_date_raises(self):
        """Malformed signed_date must raise ValueError on register."""
        t = self._tracker()
        with self.assertRaises(ValueError) as ctx:
            t.register_baa(
                destination_id="bad_dest",
                vendor="V",
                signed_date="01/15/2024",   # wrong format
                expiry_date=self._future(),
            )
        self.assertIn("signed_date", str(ctx.exception))
...
class TestIRBApprovalGate(_TmpMixin):
    ...
    def test_export_usage_report_html(self):
        ...
        self.assertTrue(out.exists())
        self.assertIn("IRB-R", out.read_text(encoding="utf-8"))
    ...
    def test_register_protocol_invalid_expiry_raises(self):
        """Malformed expiry_date must raise ValueError on register."""
        g = self._gate()
        with self.assertRaises(ValueError) as ctx:
            g.register_protocol(
                protocol_id   = "IRB-BAD",
                ...
                pi_name       = "Dr. X",
                approved_date = "2024-01-01",
                expiry_date   = "25-12-2025",  # wrong format
            )
        self.assertIn("expiry_date", str(ctx.exception))

    def test_register_protocol_invalid_approved_date_raises(self):
        """Malformed approved_date must raise ValueError on register."""
        g = self._gate()
        with self.assertRaises(ValueError) as ctx:
            g.register_protocol(
                protocol_id   = "IRB-BAD2",
                study_title   = "Bad study",
                pi_name       = "Dr. X",
                approved_date = "not-a-date",
                expiry_date   = self._future(),
            )
        self.assertIn("approved_date", str(ctx.exception))
...
class TestPHIKAnonymityChecker(_TmpMixin):
    ...
    def test_save_report_html(self):
        ...
        self.assertTrue(out.exists())
        self.assertIn("k-Anonymity", out.read_text(encoding="utf-8"))
...
class TestClarityExtractorNoDB(_TmpMixin):
    ...
    def test_refresh_window_blocked(self):
        ...
        cx = ClarityExtractor(
            ...
            refresh_window_start=1,
            refresh_window_end=4,
