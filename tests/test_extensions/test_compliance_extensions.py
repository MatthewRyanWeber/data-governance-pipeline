...
class TestComplianceMonitor(_TmpMixin):
    ...
    def test_save_report_html(self):
        ...
        self.assertIn("Compliance Controls Status", out.read_text(encoding="utf-8"))
...
class TestTrustReportGenerator(_TmpMixin):
    ...
    def test_irb_titles_hidden_by_default(self):
        ...
        out = pathlib.Path(self._tmp) / "trust_report.html"
        r.generate(out)
        self.assertNotIn("SECRET STUDY NAME", out.read_text(encoding="utf-8"))
    ...
    def test_irb_titles_shown_when_enabled(self):
        ...
        out = pathlib.Path(self._tmp) / "trust_report.html"
        r.generate(out)
        self.assertIn("VISIBLE STUDY NAME", out.read_text(encoding="utf-8"))
    ...
    def test_controls_monitor_results_included(self):
        ...
        out = pathlib.Path(self._tmp) / "trust_report.html"
        r.generate(out)
        self.assertIn("LOG_DIR_WRITABLE", out.read_text(encoding="utf-8"))
