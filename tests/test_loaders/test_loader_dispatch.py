...
class TestCockroachDBLoader(unittest.TestCase):
    ...
    def _captured_url(self, cfg, has_cockroach=False):
        ...
        import pipeline.loaders.cockroachdb_loader as _mod
        orig = _mod.HAS_COCKROACH
        _mod.HAS_COCKROACH = has_cockroach
        captured = {}
        def mock_ce(url, **kw):
            captured["url"] = url
            return MagicMock()
...
class TestNewDestinationLoaders(unittest.TestCase):
    ...
    def test_duckdb_raises_without_package(self):
        import pipeline.loaders.duckdb_loader as _mod
        orig = _mod.HAS_DUCKDB; _mod.HAS_DUCKDB = False
        try:
            with self.assertRaises(RuntimeError): DuckDBLoader(self.gov)
        finally: _mod.HAS_DUCKDB = orig
